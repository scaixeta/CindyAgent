...
```bash
INIT=$(node ".agents/get-shit-done/bin/gsd-tools.cjs" init todos)
...
```bash
slug=$(node ".agents/get-shit-done/bin/gsd-tools.cjs" generate-slug "$title" --raw)
```
...
```bash
node ".agents/get-shit-done/bin/gsd-tools.cjs" commit "docs: capture todo - [title]" --files .planning/todos/pending/[filename] .planning/STATE.md
```
