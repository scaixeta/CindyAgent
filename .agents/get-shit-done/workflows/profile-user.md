...
Key references:
- @.agents/get-shit-done/references/ui-brand.md (display patterns)
- @.agents/get-shit-done/agents/gsd-user-profiler.md (profiler agent definition)
- @.agents/get-shit-done/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH=".agents/get-shit-done/USER-PROFILE.md"
...
```bash
cp ".agents/get-shit-done/USER-PROFILE.md" ".agents/get-shit-done/USER-PROFILE.backup.md"
```
...
✓ Stores profile at .agents/get-shit-done/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(node .agents/get-shit-done/bin/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node .agents/get-shit-done/bin/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `.agents/get-shit-done/references/user-profiling.md`
...
Reference: @.agents/get-shit-done/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node .agents/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node .agents/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node .agents/get-shit-done/bin/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to .agents/get-shit-done/USER-PROFILE.md"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at .agents/get-shit-done/USER-PROFILE.md" and jump to step 10.
...
```bash
node .agents/get-shit-done/bin/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /gsd-dev-preferences at .agents/commands/gsd/dev-preferences.md"

**For GEMINI.md profile section (if selected):**

```bash
node .agents/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node .agents/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```
...
```bash
BACKUP_PATH=".agents/get-shit-done/USER-PROFILE.backup.md"
```
...
Your profile:    .agents/get-shit-done/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /gsd-dev-preferences   .agents/commands/gsd/dev-preferences.md
