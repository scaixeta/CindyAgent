...
**Endpoint**: `http://204.168.202.5:8080/api/v1/{device_token}/telemetry`
...
**Endpoint**: `http://204.168.202.5:8080/api/plugins/telemetry/DEVICE/{deviceId}/values/timeseries`
...
## Camada de Memória Operacional (Redis)
...
```
scripts/hermes_memory.py
        |
        +---> Redis (localhost:6379)
                  |
                  +---> hermes:session:active           (TTL 30 min)
                  +---> hermes:session:{id}             (TTL 30 min)
                  +---> hermes:conversation:{session}   (TTL 7 dias)
                  +---> hermes:memory:{key}             (TTL 1 dia)
                  +---> hermes:user:{user}:{pref}      (TTL 1 dia)

scripts/s5_redis_memory.py
        |
        +---> Redis (localhost:6379)
                  |
                  +---> microclimate:{device}:latest     (TTL 1h)
                  +---> microclimate:{device}:window    (TTL 1h)
                  +---> microclimate:{device}:memory_state:{marker}  (TTL 2h)
```
...
|| Script | Propósito |
|---|---|---|
| `scripts/hermes_memory.py` | Memória persistente do Hermes: sessão, conversa, preferências, contexto |
| `scripts/s5_redis_memory.py` | Camada Redis para microclima: latest state, window, memory state |

### Comandos do Hermes Memory

```bash
python3 scripts/hermes_memory.py --op status
python3 scripts/hermes_memory.py --op store_memory --key project_name --value "Sentivis SIM" --ttl 604800
python3 scripts/hermes_memory.py --op get_memory --key project_name
python3 scripts/hermes_memory.py --op append_conversation --role user --content "mensagem"
python3 scripts/hermes_memory.py --op search_conversation --query "Sprint"
python3 scripts/hermes_memory.py --op store_user_pref --key lingua --value "pt-BR"
```
...
| Graceful fallback | Script continua se Redis indisponível | Resiliência operativa |

### Sprint 5 — Relatório Agrometeorológico

#### Contexto

Sprint 5 validou o motor analítico local de microclima sobre os 12 registros reais da janela S4 (2026-04-10, 17:59-18:57 UTC, NIMBUS-AERO).

#### Arquitetura MVP

```
collection_log.txt (S4)
        |
        v
scripts/s5_microclimate.py
  + parser de 12 registos
  + cálculo de tendência
  + marcadores operativos (sem claims agronómicos)
        |
        +---> artifacts/s5-relatorio.md
        +---> artifacts/s5-relatorio.json
        +---> scripts/s5_redis_memory.py (latest + window + memory_state)
```

#### Métricas Extraídas (S4)

|| Métrica | Valor | Tendência |
|---|---|---|
| Temperatura | 25.0°C | Estável |
| Umidade | 87.11% | Subida (64.45% -> 88.28%) |
| Proxy molhamento | 10/12 pontos acima de 85% | — |

#### Regras Operativas Seguras

|| Formulação | Status |
|---|---|---|
| Temperatura actual: X°C | Confirmado |
| Temperatura estável / subindo / descendo | Confirmado |
| Umidade actual: X% | Confirmado |
| Umidade em tendência de subida / descida / estável | Confirmado |
| Intervalo de referência operativa: 18-24°C | Heurística — não validação agronómica |
| Proxy de molhamento foliar: umidade >85% | Heurística operativa |
| "Estresse térmico confirmado" | Bloqueado |
| "Risco de ferrugem" | Bloqueado |
| "Janela de chuva confirmada" | Bloqueado |
| VPD | Bloqueado (Buck equation não existe no cânon) |

#### Sprint 5 — Estórias e Resultados

|| Story | SP | Estado |
|---|---|---|---|
| ST-S5-01 | 3 | Concluído |
| ST-S5-02 | 5 | Concluído |
| ST-S5-03 | 3 | Concluído |
| ST-S5-04 | 5 | Concluído |
| ST-S5-05 | 3 | Concluído |
| ST-S5-06 | 3 | Concluído |
...
| Labels de Tipo | `história`, `bug`, `change_request` | Contrato operacional da Cindy |
